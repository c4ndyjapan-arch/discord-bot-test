...
- #bot-chat 채널에서 봇 제외 모든 메시지에 자동 응답
- 봇 소유자가 DM으로 !api <키> 명령어로 Gemini API 키 설정
- char-description.txt를 시스템 프롬프트로 사용
"""
...
def load_config() -> dict:
    """config.json을 읽어 반환합니다. 없으면 기본값을 반환합니다."""
...
async def get_gemini_response(channel_id: int, user_message: str):
    """Gemini API를 호출하여 응답을 생성합니다. API 키 미설정 시 None 반환."""
    config = load_config()
...
# Discord 봇 설정 (message_content 인텐트 필요)
...
async def on_ready():
    ...
    if config.get("gemini_api_key"):
        ...
        print("Gemini API 키: 미설정 ⚠️  봇에게 DM으로 '!api YOUR_KEY'를 전송해주세요.")
    print("봇이 준비되었습니다!")
...
async def on_message(message: discord.Message):
    ...
    # DM 채널: 소유자의 !api 명령어만 처리
    if isinstance(message.channel, discord.DMChannel):
        if message.author.id == OWNER_ID and message.content.startswith("!api "):
            new_key = message.content[5:].strip()
            if new_key:
                config = load_config()
                config["gemini_api_key"] = new_key
                save_config(config)
                await message.reply("✅ API 키가 성공적으로 저장되었습니다.", mention_author=False)
            else:
                await message.reply("⚠️ API 키를 입력해주세요. 예: `!api YOUR_KEY`", mention_author=False)
        return
    ...
    async with message.channel.typing():
        try:
            ...
            if reply is None:
                await message.reply(
                    "⚠️ API 키가 설정되지 않았습니다. 봇 관리자에게 문의하세요.",
            ...
            await message.reply(f"오류가 발생했습니다: {str(e)}", mention_author=False)
